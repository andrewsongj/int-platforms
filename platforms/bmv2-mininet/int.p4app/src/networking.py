...
def start_int_collector(influxdb):
    ...
    pg_install_cmd = "pip install psycopg2==2.6.1"
    print(pg_install_cmd)
    os.system(pg_install_cmd)

    # collector_cmd = "ip netns exec ns_int python3 /tmp/utils/int_collector_influx.py -i 6000 -H 192.168.0.1:8086 -d 0 &> /dev/null"
    collector_cmd = "ip netns exec ns_int python3 /tmp/utils/int_collector_postgres.py -i 6000 -H 192.168.0.1:5432 -d 0 &> /dev/null"
    print(collector_cmd)
...
def create_dp_cpu_link(id, switch, bridge_name):
    ...
    for off in "rx tx sg tso ufo gso gro lro rxvlan txvlan rxhash".split(" "):
        ...
        quietRunNs(
            "/sbin/ethtool --offload veth_cpu_%i %s off" % (id, off), display=False
        )
