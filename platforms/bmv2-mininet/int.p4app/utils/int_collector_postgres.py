...
logging.basicConfig(
    level=logging.ERROR, format=log_format, filename="/tmp/p4app_logs/int_collector.log"
)
...
def parse_params():
    ...
    parser.add_argument(
        "-H", "--host", default="localhost", help="InfluxDB server address"
    )

    parser.add_argument(
        "-D", "--database", default="int_telemetry_db", help="Database name"
    )
...
class HopMetadata:
    ...
    def __parse_ports(self):
        if self.ins_map & 0x40:
            ...
            logger.debug(
                "parse ingress port: %d, egress_port: %d"
                % (self.l1_ingress_port_id, self.l1_egress_port_id)
            )
    ...
    def __parse_queue_occupancy(self):
        if self.ins_map & 0x10:
            ...
            logger.debug(
                "parse queue_occupancy_id: %d, queue_occupancy: %d"
                % (self.queue_occupancy_id, self.queue_occupancy)
            )
    ...
    def __parse_queue_congestion(self):
        if self.ins_map & 0x02:
            self.queue_congestion_id = int.from_bytes(
                self.data.read(1), byteorder="big"
            )
            self.queue_congestion = int.from_bytes(self.data.read(3), byteorder="big")
            logger.debug(
                "parse queue_congestion_id: %d, queue_congestion: %d"
                % (self.queue_congestion_id, self.queue_congestion)
            )
    ...
    def __parse_l2_ports(self):
        if self.ins_map & 0x02:
            ...
            logger.debug(
                "parse L2 ingress port: %d, egress_port: %d"
                % (self.l2_ingress_port_id, self.l2_egress_port_id)
            )

    def __parse_egress_port_tx_util(self):
        if self.ins_map & 0x01:
            self.egress_port_tx_util = int.from_bytes(
                self.data.read(4), byteorder="big"
            )
...
class IntReport:
    def __init__(self, data):
        ...
        if self.ver != 1:
            logger.error(
                "Unsupported INT report version %s - skipping report" % self.int_version
            )
            raise Exception(
                "Unsupported INT report version %s - skipping report" % self.int_version
            )
        ...
        self.rep_md_bits = (self.int_report_hdr[1] & 0x1F) + (
            self.int_report_hdr[2] >> 7
        )
        ...
        self.switch_id, self.seq_num, self.ingress_tstamp = struct.unpack(
            "!3I", orig_data[4:16]
        )
        ...
        if self.int_version == 0:  # if rep is 0 then it is ok for INT v0.4
            ...
            logger.debug(
                "hop_metadata_len: %d, int_data_len: %d, remaining_hop_cnt: %d, hop_count: %d"
                % (
                    self.hop_metadata_len,
                    self.int_data_len,
                    self.remaining_hop_cnt,
                    self.hop_count,
                )
            )
        else:
            logger.error(
                "Unsupported INT version %s - skipping report" % self.int_version
            )
            raise Exception(
                "Unsupported INT version %s - skipping report" % self.int_version
            )
        ...
        logger.debug(
            "Metadata (%d bytes) is: %s"
            % (len(self.int_meta), binascii.hexlify(self.int_meta))
        )
        self.hop_metadata = []
        ...
        for i in range(self.hop_count):
            try:
                ...
                logger.info(
                    "Metadata left (%s position) is: %s"
                    % (self.int_meta.tell(), self.int_meta)
                )
                logger.error(e)
                break
    ...
    def __str__(self):
        ...
        for hop in self.hop_metadata:
            hop_info += str(hop) + "\n"
        flow_tuple = (
            "src_ip: %(srcip)s, dst_ip: %(dstip)s, src_port: %(scrp)s, dst_port: %(dstp)s, protocol: %(protocol)s"
            % self.flow_id
        )
        additional_info = (
            "sw: %s, seq: %s, int version: %s, ins_map: 0x%x, hops: %d"
            % (
                self.switch_id,
                self.seq_num,
                self.int_version,
...
class IntCollector:
    def __init__(self, influx, period):
        ...
        self.last_hop_ingress_timestamp = (
            {}
        )  # save last ingress timestamp per each hop in each monitored flow
    ...
    def add_report(self, report):
        ...
        if reports_cnt > 100 or time.time() - self.last_send > self.period:
            logger.info(
                "Sending %d reports to influx from last %s secs"
                % (reports_cnt, time.time() - self.last_send)
            )
            self.__send_reports()
    ...
    def __prepare_e2e_report(self, report, flow_key):
        ...
        json_report = {
            ...
            "time": int(
                time.time() * 1e9
            ),  # use local time because bmv2 clock is a little slower making time drift
            "fields": {
        ...
        if flow_key in self.last_dstts:
            json_report["fields"]["sink_jitter"] = (
                1.0 * destination_timestamp - self.last_dstts[flow_key]
            )

        # add reordering only if can be calculated (not first packet in the flow)
        if flow_key in self.last_reordering:
            json_report["fields"]["reordering"] = (
                1.0 * report.seq_num - self.last_reordering[flow_key] - 1
            )
    ...
    def __prepare_hop_report(self, report, index, hop, flow_key):
        ...
        if flow_hop_key in self.last_hop_ingress_timestamp:
            json_report["fields"]["hop_jitter"] = (
                hop.ingress_timestamp - self.last_hop_ingress_timestamp[flow_hop_key]
            )
        ...
        if "ingress_timestamp" in vars(hop) and index > 0:
            json_report["fields"]["link_delay"] = (
                hop.ingress_timestamp - self.last_hop_delay
            )
    ...
    def __prepare_reports(self, report):
        flow_key = (
            "%(srcip)s, %(dstip)s, %(scrp)s, %(dstp)s, %(protocol)s" % report.flow_id
        )
        reports = []
...
def start_udp_server(args):
    ...
    while True:
        ...
        logger.info(
            "Received INT report (%d bytes) from: %s" % (len(message), str(address))
        )
